...
BEGIN
    ...
    WHILE i < column_count DO
        SET col_name = CONCAT('col', i);
        SET col_type = IF(MOD(i, 2) = 0, 'INT', 'VARCHAR(1024)');
    ...
    SET create_table_sql = CONCAT(create_table_sql, ') ENGINE=NDB;');
    ...
    SET i = 0;
    WHILE i < column_count DO
        SET insert_sql = CONCAT(insert_sql, 'col', i, ', ');
        SET i = i + 1;
    END WHILE;
		-- Remove last comma
    ...
    WHILE row_id <= row_count DO
        ...
        WHILE i < batch_size AND row_id <= row_count DO
            ...
            WHILE j < column_count DO
                IF MOD(j, 2) = 0 THEN
                    SET @row_values = CONCAT(@row_values, FLOOR(RAND() * 10000), ', ');
                ELSE
